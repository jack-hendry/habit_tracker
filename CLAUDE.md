...
- End-of-feature ritual, in order: (1) update any docs the feature touched, (2) write the `AD`/`B` entries and the `L` headlines the spec produced into `specs/STATE.md`, with each lesson's body in `specs/LESSONS.md`, (3) **summarize the spec down to what matters** — the final `Analyst.md`, the decisions, and what changed from the plan; delete drafts and long back-and-forth notes so the archived folder is small, (4) move the summarized folder to `specs/archive/YYYY-MM-DD-<name>/` and push both the archive move and the `STATE.md` update together. A `pre-push` git hook (`scripts/clean-table-check.sh`, wired via `.pre-commit-config.yaml`) enforces steps 2–4: it blocks a push that adds files under `specs/archive/` without a matching `specs/STATE.md` change, and blocks any newly archived file over 20 KB with "Summarize this spec before archiving." A third gate runs on **every** push, not just archive pushes: every `L-NNN` headline in `specs/STATE.md` must have a body in `specs/LESSONS.md` and vice versa, so the two cannot drift. The hook checks — it never edits or commits for you. It's a client-side git hook, so `git push --no-verify` bypasses it entirely; it's a reminder for the honest case, not a guarantee. A PreToolUse hook (`.claude/hooks/warn-push-bypass.sh`) makes that bypass visible — it asks before any `git push --no-verify`, from the top-level session or a subagent, whether or not a run is in flight. It only surfaces the bypass; it cannot stop one. **CI is the guarantee**: `.github/workflows/ci.yml` runs the *same script* on every pull request and every push to `main`, alongside a `tests` job running the Karma suite. Never reimplement a gate's rules in the workflow YAML — one script, two callers, or the hook and CI drift into disagreeing about what "clean" means (AD-034). A fourth gate, also on **every** push, checks `specs/STATE.md` against its ~300-line size budget by calling `tools/state_rotate.py --check`; the hook only ever runs `--check`, never `--apply`, so it reports and refuses but never rewrites your memory file. CI has a third job, `lint`, running `ruff` over `tools/`.
- Before starting a `tasks.md` run, add a `## Executing: <spec-dir-name>` line to the root `STATE.md`, and remove it when the run finishes. The `enforce-haiku-tasks-pretooluse.sh` hook keys off that marker for three gates — without it the hook stays dormant, so unrelated Small tasks are never blocked. While the marker is live it (1) **denies** a direct top-level edit to project code, (2) **asks** before `git commit` / `git push` from the top-level session, since the marker is what says the tree is mid-run, and (3) **asks** before dispatching a subagent with `model: opus` or with no `model` at all — `TEMPLATE.md` allows only `haiku`, `sonnet` or `top-level` for a step, and an absent field inherits opus. Gates 2 and 3 are `ask`, not `deny`: a checkpoint commit before merging worktree branches and a deliberate one-off dispatch are both legitimate. Subagents are exempt from all three, which is what preserves the worktree-branch commit carve-out above. `.claude/hooks/enforce-haiku-tasks.test.sh` is the matrix; run it after touching the hook.
...
- `python3 tools/state_rotate.py --check` — report whether `specs/STATE.md` is over its ~300-line budget and which aged `AD`/`B` entries would rotate. Exits non-zero when over; this is what the pre-push hook and CI run.
- `python3 tools/state_rotate.py --apply` — actually move them into `specs/STATE-ARCHIVE.md`, leaving a one-line stub each. Oldest-first by the entry's own date, stopping as soon as the file fits. Lesson headlines never move. Run this yourself; no hook ever runs it for you.
- `ruff check . && ruff format --check tools/` — lint the Python tooling (`ruff.toml`, scoped to `tools/`). CI's `lint` job runs the same two commands.
