...
if (process.argv.includes('--json')) {
  ...
  console.log(`localStorage.setItem(${JSON.stringify(STORAGE_KEY)}, ${JSON.stringify(JSON.stringify(habits))}); location.reload();`);
  console.error(`\n^ paste that into the DevTools console on the running app (${habits.length} habits, ~18 weeks of history).`);
  console.error('To clear it again: localStorage.removeItem("habit_tracker.habits.v1"); location.reload();');
}
